...
import { formatProduct, formatPrice } from '../utils/formatters';
import { notifyAdmins, sendOrderConfirmation } from '../services/notification';

/**
 * Order conversation flow
 */
export async function orderConversation(conversation: Conversation<BotContext, BotContext>, ctx: BotContext) {
  try {
    ...
    // Get product details
    const product = await apiClient.getProduct(orderData.productId);
    ...
    if (!phone) {
      ...
      await ctx.reply(
        '📞 Введите номер телефона для связи (в формате +79991234567):',
    ...
    const confirmText = `📋 **Подтвердите заказ**\n\n` +
      ...
      `Цена: ${formatPrice(basePrice)} x ${quantity} = ${formatPrice(totalAmount)}\n\n` +
      `📞 Телефон: ${phone}\n` +
      `📍 Адрес: ${address}\n` +
    ...
    try {
      ...
      await sendOrderConfirmation(ctx.api, ctx.from!.id, order);

      // Notify admins
      await notifyAdmins(ctx.api, order);
    ...
    logger.error('Order conversation error:', error);
